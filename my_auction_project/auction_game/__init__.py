...
class Group(BaseGroup):
    ...
    gemini_bid = models.CurrencyField()


class Player(BasePlayer):
    chosen_bid_type = models.StringField(blank=True)
    ...
    adjusted_bid = models.CurrencyField(blank=True, null=True)  # neu hinzugefügt
...
def build_prompt(strategy, valuation_info, player, all_bids_summary, all_payoffs_summary):
    ...
    if strategy == "basic":
        prompt = (
            base_context +
            "What would you suggest as a bid in this situation? Feel free to include your reasoning."
...
def set_payoffs(group: Group):
    ...
    for p in players:
        if p == winner:
            p.is_winner = True
            p.pyoff = C.BUDGET + p.valuation - p.bid
        else:
            p.is_winner = False
            p.pyoff = C.BUDGET
        bid_history.append({
...
class GeminiChoice(Page):
    ...
    def vars_for_template(player: Player):
        ...
        '''
        player_gemini_aggressive = sc.get(f"player{player.id_in_group}_geministrategy_aggressive", False)  # N
        ...
        '''
        enable_gemini = any(
            strategy in allowed_options_for_player for strategy in ["basic", "risk_averse", "aggressive"])
...
class FinalResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        player.payoff = final_payoff_in_game_points
        return dict(
